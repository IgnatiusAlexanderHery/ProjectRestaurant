...
public class CustomerController {
	...
	public CustomerController(OwnerController data) {
	...
	public void runProgram(Customer customer) {
		...
		do {
			try {
				customer = listCustomer.get(getIndexByID(customer.getId()));
				System.out.println();
				System.out.println("Welcome "+ customer.getName());
				showMenu.MenuCustomer();
				System.out.print("Choice >> ");
				option = input.nextInt();
				if(option == 1) {
					updateProfile(customer.getId());
				}else if (option == 2) {
					Pesanan(customer);
					input.nextLine();
				}else if (option == 3) {
	...
	private void Pesanan(Customer customer) {
		// TODO Auto-generated method stub
		showPesanan.runProgram(customer);
	...
	private boolean checkDuplicateUsername(String data) {
		for(int i = 0; i < listCustomer.size(); i++) {
			if(listCustomer.get(i).getUsername().equals(data)) {
				return true;
			}
		}
		return false;
	...
	private void setProfile(String id) {
	    ...
	    Customer cusrtomertemp = listCustomer.get(getIndexByID(id));
		cusrtomertemp.setUsername(username);
		cusrtomertemp.setPassword(password);
		cusrtomertemp.setEmail(email);
		cusrtomertemp.setGender(gender);
		cusrtomertemp.setAddress(address);
		cusrtomertemp.setName(name);
	}
